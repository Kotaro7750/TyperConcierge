...
export function QueryPane(props: { queryInfo: QueryInfo, paneInfo: QuerySentencePaneInfo }): JSX.Element {
  ...
  // タイピング終了時にNaNになって不安定になるのを防ぐために最大でも最後のインデックスよりも大きくならないようにしている
  const viewStrCursorPosMin = Math.min(...viewStrCursorPos, queryInfo.viewString.length - 1);
  ...
  const elemList = constructStyledStringElement(queryInfo.viewString, viewStrCursorPos, viewStrMissedPos, viewStrLastPos);
  ...
  for (let i = 0; i < elemList.length; ++i) {
    ...
    if (inLineUnitCount >= limitASCIICount.current) {
      ...
      lineList.push(<div className='d-flex justify-content-between'>{...inLineElemList}</div>);
      inLineElemList = [];
      inLineUnitCount = 0;
    }
  }
  ...
  if (inLineElemList.length != 0) {
    ...
    lineList.push(<div className='d-flex justify-content-start'>{...inLineElemList}</div>);
  }
  ...
  useEffect(() => {
    ...
    const unitWidth = isMonoWidthFont(queryInfo.viewString[viewStrCursorPosMin]) ? spanWidth : spanWidth / 2;
    // ボックスからパディングを抜いた幅を取得
